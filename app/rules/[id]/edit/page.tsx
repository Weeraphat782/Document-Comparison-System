...
import { COMMON_DOCUMENT_TYPES, COMMON_EXTRACTION_FIELDS, ComparisonRule } from '@/lib/types';
...
export default function EditRulePage() {
  ...
  async function loadRule() {
    try {
      ...
      if (response.ok) {
        const rule: ComparisonRule = await response.json();
        setName(rule.name);
        setDescription(rule.description || '');
        setExtractionFields(rule.extraction_fields);
        setComparisonInstructions(rule.comparison_instructions);
        setCriticalChecks(rule.critical_checks);
      ...
      setError('An error occurred while loading the rule');
    } finally {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (response.ok) {
        router.push('/rules?updated=true');
      } else {
      ...
      <div className="container mx-auto px-4 py-8">
        <Card className="border-destructive">
          <CardContent className="pt-6">
            <div className="flex items-center gap-2 text-destructive mb-4">
              <AlertCircle className="h-5 w-5" />
              <span className="font-semibold">Cannot Edit Default Rule</span>
            </div>
            <p>This is a default rule and cannot be edited.</p>
            <Link href="/rules">
              <Button className="mt-4">
                Back to Rules
              </Button>
    ...
    <div className="container mx-auto px-4 py-8 max-w-4xl">
      ...
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">
          Modify the rule settings and comparison logic
        </p>
      </div>
